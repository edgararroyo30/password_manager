...
def split_key(data, chunk_size):
    return [data[i:i+chunk_size] for i in range(0, len(data), chunk_size)]


def store_large_encryption_key(service_name, key_name, key_data, chunk_size=500):
    key_parts = split_key(key_data.encode('utf-8'), chunk_size)
...
def check_keys_exist(service_name):
...
def store_encryption_keys(service_name, private_key, public_key):

    if not check_keys_exist(service_name):
        store_large_encryption_key (service_name, "private_key", private_key)
...
private_key_passwords, public_key_passwords = create_keys()

store_encryption_keys("Password_keys", private_key_passwords, public_key_passwords)
